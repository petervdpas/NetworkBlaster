    ...
    private async Task<(string token, DateTimeOffset expiresAt)> FetchAsync(CancellationToken ct)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            throw new HttpRequestException(
                $"NetworkBlast.OAuth2: token endpoint returned {(int)response.StatusCode}. Body: {body}");
        }
        ...
        var payload = await System.Text.Json.JsonSerializer
            ...
            ?? throw new HttpRequestException("NetworkBlast.OAuth2: token endpoint returned an empty body.");

        if (string.IsNullOrEmpty(payload.AccessToken))
            throw new HttpRequestException("NetworkBlast.OAuth2: token endpoint response missing access_token.");
